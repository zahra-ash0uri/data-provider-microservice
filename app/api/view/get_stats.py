...
class GetStats(RequestHandler):
    ...
    def get(self):
        ...
        try:
            stats_dataframe = mongo_adapter.system_requests_status_dataframe
        except:
            self.terminate_request(status_code=503, message=AppVO.ErrorMessages.service_unavailable,
                                   received_at=received_at)

        if stats_dataframe is not None:
            totals = mongo_adapter.count_system_requests_status_collection
    ...
    def update_system_requests_status(received_at, response_time):
        mongo_adapter.insert_new_status(received_at=received_at, response_time=response_time)
